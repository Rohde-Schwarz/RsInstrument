...
- Log only errors and skip problem-free parts; this way you avoid going through thousands of lines of texts
...
Very often, you do not need the absolute time in logging, but rather the relative times from the beginning. This way you quickly see the duration of your procedure. To set this up, use the constructor's option string ``LoggingRelativeTimeOfFirstEntry=True`` or the method ``set_time_offset_zero_on_first_entry()``. Another nice feature is time statistic - instrument execution time and total time. These work independent of the fact, whether the logger is running or not:
...
We hope you are a happy Rohde & Schwarz customer, and hence you use more than one of our instruments. In such case, you probably want to log from all the instruments into a single target (file). Therefore, you open one log file for writing (or appending) and set is as the logging target for all your sessions:
