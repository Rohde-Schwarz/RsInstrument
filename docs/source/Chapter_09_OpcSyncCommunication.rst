...
Now we are getting to the cool stuff: OPC-synchronized communication. OPC stands for OPeration Complete. The idea is: use one method (write or query), which sends the command, and polls the instrument's status subsystem until it indicates: **"I'm finished"**. The main advantage is, you can use this mechanism for commands that take several seconds, or minutes to complete, and you are still able to interrupt the process if needed. You can also perform other operations with the instrument in a parallel thread.
...
.. literalinclude:: Example_MethodsWithOpc.py
   :language: python
   :linenos:
