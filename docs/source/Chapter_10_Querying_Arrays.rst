...
.. literalinclude:: Example_QueryFloatArray_ASCII.py
   :language: python
   :linenos:
...
One additional setting we need though - the binary data from the instrument does not contain information about its encoding. Is it 4 bytes float, or 8 bytes float? Little Endian or Big Endian? This, we specify with the property ``bin_float_numbers_format``:

.. literalinclude:: Example_QueryFloatArray_Bin.py
   :language: python
   :linenos:
...
.. literalinclude:: Example_QueryIntArray_AsciiBin.py
   :language: python
   :linenos: